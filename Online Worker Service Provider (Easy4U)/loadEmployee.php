...
if(isset($_POST['searchButton'])){
  ...
  $sqlUserCheck="SELECT * FROM employee where admin=0 LIMIT $startingRow, $perPage;";
  $result=mysqli_query($conn, $sqlUserCheck);
  //$row=mysqli_num_rows($result);
}
  $sqlRowCount="SELECT * FROM employee where admin=0";
...
{ 
  ...
  if(!empty($_POST['userName'])){
   ...
   if(isset($_POST['updateButton'])){   
     $sqlUserCheckToUpdate="SELECT * FROM employee where userName = '$userName'";
    ...
    $sql = "UPDATE employee SET firstName='$firstName', lastName='$lastName', userName='$userName', email='$email', phone='$phone', NID='$NID', address='$address', DOB='$DOB', salary='$salary' WHERE userName = '$userName'";
...
if(isset($_POST['addButton'])){   
     ...
     $rowUser=mysqli_num_rows($resultUser);
    if($rowUser < 1){
      $sql = "INSERT INTO employee (firstName, lastName, userName, email, phone, NID, address, DOB, salary)
              VALUES ('$firstName', '$lastName', '$userName', '$email', '$phone', '$NID', '$address', '$DOB', '$salary');";
...
if(isset($_POST['refreshButton'])){ 
      header("Refresh:0; url=loadEmployee.php");
    }
      ...
      <div class="container">
        ...
        <nav>
          <ul>
            ...
            <li class="current"><a href="loadEmployee.php">Employee</a></li><br><br>
...
<form id="loadCustomer" action="loadEmployee.php" method="post">
     <section id="Customerload">
              ...
              var table = document.getElementById("show");
                for(var i = 1; i < table.rows.length; i++)
                {
                    table.rows[i].onclick = function()
                    {
                         ...
                         document.getElementById("salary").value = this.cells[8].innerHTML;
                    };
                  }
       ...
       <div id="buttonpg">
       <a href="loadEmployee.php?prev=<?php echo $a-1; ?>" class="buttonpg">Previous</a>
       <?php
      for($i=1; $i<=$totalPage; $i++){ ?>
       <a href="loadEmployee.php?pg=<?php echo $i; ?>" class="buttonpg"><?php echo $i; ?></a>
       <?php
      }
      ?>
       <a href="loadEmployee.php?next=<?php echo $a+1; ?>" class="buttonpg">Next</a>
     </div>
