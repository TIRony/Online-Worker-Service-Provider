...
if (count($_POST) > 0) {
    ...
    if ($_POST["currentPhone"] == $row["phone"]) {
        ...
        mysqli_query($conn, "UPDATE employee set phone='" . $_POST["newPhone"] . "' WHERE userName='" . $_SESSION["userName"] . "'");
...
if (count($_POST) > 0) {
    $result = mysqli_query($conn, "SELECT * from employee WHERE userName='" . $_SESSION["userName"] . "'");
    $row = mysqli_fetch_array($result);
    if ($_POST["currentEmail"] == $row["email"]) {
       
        mysqli_query($conn, "UPDATE employee set email='" . $_POST["newEmail"] . "' WHERE userName='" . $_SESSION["userName"] . "'");
      ...
      <div class="container">
        ...
        <nav>
          <ul>
            ...
            <li class="current" ><a href="AdminSetting.php">Settings</a></li><br><br>
          </ul>
        </nav>
      </form>
       </div>
